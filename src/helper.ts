...
export class LambdaHelper extends BaseClass implements ILambdaHelper {
   ...
   */
  public async DisableDynamoEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
    return await this.UpdateEventSourceMappingAsync(
      functionName,
      false,
      uuid,
   ...
   * @param batchSize {number} Batch size for the event source mapping. Default 100
   */
  public async DisableKinesisEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
   ...
   */
  public async DisableSQSEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
   ...
   * @param batchSize {number} Batch size for the event source mapping. Default 100
   */
  public async EnableDynamoEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
   ...
   * @param batchSize {number} Batch size for the event source mapping. Default 100
   */
  public async EnableKinesisEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
   ...
   * @param batchSize {number} Batch size for the event source mapping. Default 10
   */
  public async EnableSQSEventSourceMappingAsync(
    functionName: string,
    uuid: string,
    batchSize?: number,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
   ...
   */
  public async GetEventSourceMappingAsync(
    uuid: string,
  ): Promise<Lambda.EventSourceMappingConfiguration> {
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(uuid)) {
      throw new Error(`[${action}]-Must supply uuid`);
    }
   ...
   */
  public async ListEventSourceMappingsAsync(
    functionName: string,
    ...
    // guard clauses
    if (this.ObjectOperations.IsNullOrWhitespace(functionName)) {
      throw new Error(`[${action}]-Must supply functionName`);
    }
    if (this.ObjectOperations.IsNullOrWhitespace(eventSourceArn)) {
      throw new Error(`[${action}]-Must supply eventSourceArn`);
    }
   ...
   */
  public async UpdateEventSourceMappingAsync(
    functionName: string,
    enabled: boolean,
    uuid: string,
    batchSize: number,
